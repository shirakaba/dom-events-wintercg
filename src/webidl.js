...
import { kEmptyObject } from './util.js';

export const converters = { __proto__: null };
...
export function convertToInt(name, value, bitLength, options = kEmptyObject) {
  ...
  if (enforceRange) {
    ...
    if (NumberIsNaN(x) || x === Infinity || x === -Infinity) {
      throw new codes.ERR_INVALID_ARG_VALUE(name, x);
    }
...
converters.DOMString = function DOMString(V) {
  if (typeof V === 'symbol') {
    throw new codes.ERR_INVALID_ARG_VALUE('value', V);
  }
...
export function makeException(message, opts = kEmptyObject) {
  const prefix = opts.prefix ? opts.prefix + ': ' : '';
  const context =
    opts.context?.length === 0 ? '' : (opts.context ?? 'Value') + ' ';
  return codedTypeError(`${prefix}${context}${message}`, {
